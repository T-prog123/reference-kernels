...
void qr_v8_kernel(const float* A, float* H, float* tau, int batch, int n) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("qr_v8", &qr_v8, "blocked v8 QR clean compile path");
}
"""

    _qr_v8_ext = load_inline(
        name="qr_v2_blocked_v8_clean_ext",
        cpp_sources=[cpp_src],
        cuda_sources=[_QR_V8_CUDA_SRC],
        extra_cflags=["-O3"],
