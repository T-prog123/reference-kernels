    ...
    int ib)
{
    ...
    (void)cublasSgemmStridedBatched(
        handle,
    ...
    int n)
{
    ...
    size_t acol_elems = total_elems;
    size_t v_elems = static_cast<size_t>(batch) * n * kPanelSize;
    size_t workspace_elems = acol_elems + v_elems;

    float* workspace = nullptr;
    (void)cudaMalloc(&workspace, workspace_elems * sizeof(float));
    ...
    int n)
{
    ...
    size_t acol_elems = total_elems;
    size_t v_elems = static_cast<size_t>(batch) * n * kPanelSize;
    size_t g_elems = static_cast<size_t>(batch) * kPanelSize * kPanelSize;
    size_t t_elems = static_cast<size_t>(batch) * kPanelSize * kPanelSize;
    size_t w_elems = static_cast<size_t>(batch) * kPanelSize * n;
    size_t w2_elems = static_cast<size_t>(batch) * kPanelSize * n;
    size_t panel_norms_elems = has_multiblock_panel
        ? static_cast<size_t>(batch) * max_tile_count
        : 0;
    size_t panel_dots_elems = has_multiblock_panel
        ? static_cast<size_t>(batch) * max_tile_count * kPanelSize
        : 0;
    size_t panel_scalars_elems = has_multiblock_panel
        ? static_cast<size_t>(batch) * 2
        : 0;
    size_t workspace_elems = acol_elems + v_elems + g_elems + t_elems +
                             w_elems + w2_elems + panel_norms_elems +
                             panel_dots_elems + panel_scalars_elems;
    ...
    if (has_multiblock_panel) {
        panel_norms = cursor;
        cursor += panel_norms_elems;
        panel_dots = cursor;
        cursor += panel_dots_elems;
        panel_scalars = cursor;
    }
...
void qr_v9_kernel(const float* A, float* H, float* tau, int batch, int n) {
...
std::vector<torch::Tensor> qr_v9(torch::Tensor A) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("qr_v9", &qr_v9, "blocked v9 QR");
}
"""

    _qr_v9_ext = load_inline(
        name="qr_v2_blocked_v9_ext",
        cpp_sources=[cpp_src],
        cuda_sources=[_QR_V9_CUDA_SRC],
        extra_cflags=["-O3"],
...
def custom_kernel(data: input_t) -> output_t:
    h, tau = _load_qr_v9_ext().qr_v9(data)
    return h, tau
