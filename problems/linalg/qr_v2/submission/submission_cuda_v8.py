...
std::vector<torch::Tensor> qr_v8(torch::Tensor A);

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("qr_v8", &qr_v8, "blocked v8 QR");
}
"""

    kernel_src = _BLOCKED_V8_CU
    kernel_src = kernel_src.replace('#include "../qr_kernel.h"\n', "")

    sig_start = kernel_src.find("template <typename>\nstruct sixth_arg;")
    sig_end = kernel_src.find("#define CHECK_CUDA_LOCAL", sig_start)
    if sig_start >= 0 and sig_end > sig_start:
        kernel_src = kernel_src[:sig_start] + kernel_src[sig_end:]

    old_entry = """void qr_custom_kernel_cuda(const float* A, float* H, float* tau, int batch, int n,
                           ignored_launch_arg) {"""
    new_entry = """void qr_v8_kernel(const float* A, float* H, float* tau, int batch, int n) {"""
    kernel_src = kernel_src.replace(old_entry, new_entry)
...
"""
    ...
    _qr_v8_ext = load_inline(
        name="qr_v2_blocked_v8_ext",
        cpp_sources=[cpp_src],
...
def custom_kernel(data: input_t) -> output_t:
    h, tau = _load_qr_v8_ext().qr_v8(data)
    return h, tau
