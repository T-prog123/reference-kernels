...
void print_profile_report() {
    ...
    std::printf("\nblocked_v7_profile phase breakdown\n");
    std::printf("note: diagnostic build; phase timing synchronizes after each measured phase.\n");
    std::printf("note: path is selected by n: tiny<=%d, small-no-T<=%d, otherwise cublas.\n",
                kTinySinglePanelCutoff, kSmallNoTCutoff);
    std::printf("%7s %6s %7s %7s %7s %11s %8s %8s %8s %8s %8s %8s %8s %8s %8s %8s %11s\n",
                "batch", "n", "path", "calls", "panels", "gpu_ms/call",
                "tiny%", "panel%", "dirUpd%", "gram%", "Trec%", "VtC%", "TW%",
                "upd%", "layout%", "other%", "host_alloc");
    ...
    for (int i = 0; i < g_profile_record_count; ++i) {
        ...
        double gpu_ms = r.tiny_ms + layout_ms + r.panel_ms + r.direct_update_ms +
                        r.gram_ms + r.t_recur_ms + r.gemm_vtc_ms +
                        r.gemm_tw_ms + r.gemm_update_ms;
        ...
        std::printf("%7d %6d %7s %7lld %7lld %11.4f %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %11.4f\n",
                    r.batch, r.n, path_name(r), r.calls, r.panels,
                    gpu_ms / calls,
